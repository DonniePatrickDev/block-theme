...
class Merlin {
	...
	public function ignore() {
		...
		if ( ! isset( $_GET['_wpnonce'] ) || ( ! wp_verify_nonce( wp_unslash( sanitize_key( $_GET['_wpnonce'] ) ), 'merlinwp-ignore-nounce' ) || ! is_admin() || ! isset( $_GET[ $this->ignore ] ) || ! current_user_can( 'manage_options' ) ) ) {
			return;
		}
	...
	protected function plugins() {
		...
		$method = '';
		if ( ! empty( $_POST['merlin_nonce'] ) && ! wp_verify_nonce( sanitize_key( $_POST['merlin_nonce'] ), 'wpnonce' ) ) {
			// Nonce is invalid.
			die( 'Security check' );
		}
	...
	protected function content() {
		...
		<form action="" method="post" class="<?php echo esc_attr( $multi_import ); ?>">
			...
			<ul class="merlin__drawer merlin__drawer--import-content js-merlin-drawer-import-content">
				<?php $import_steps_html = $this->get_import_steps_html( $import_info ); ?>
				<?php echo wp_kses_post( $import_steps_html ); ?>
			</ul>
	...
	protected function ready() {
		...
		<div class="merlin__content--transition">
			...
			<p><?php wp_kses( printf( wp_kses_post( $paragraph ), wp_kses_post( $author ) ), $allowed_html_array ); ?></p>
	...
	public function ajax_plugins() {
		...
		if ( $json ) {
			$this->logger->debug(
				...
				array(
					'plugin_slug' => wp_unslash( sanitize_key( $_POST['slug'] ) ),
	...
	public function ajax_content() {
		...
		$json         = false;
		$this_content = $content[ wp_kses_post( $_POST['content'] ) ];
		...
		if ( $json ) {
			...
			$this->logger->error(
				...
				array(
					...
					'importing_content'      => wp_kses_post(  $_POST['content'] ),
