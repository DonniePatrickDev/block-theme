...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
	...
	class TGM_Plugin_Activation {
		...
		public function admin_init() {
			...
			if ( isset( $_REQUEST['tab'] ) && 'plugin-information' === $_REQUEST['tab'] ) {
				...
				add_filter(
					'body_class',
					function( $classes ) {
						if ( is_plugin_information() ) {
							$classes[] = 'plugin-information';
						}
						return $classes;
					}
				);
		...
		protected function do_plugin_install() {
			...
			$slug = sanitize_key( urldecode( filter_input( INPUT_GET, 'plugin', FILTER_SANITIZE_STRING ) ) );
			// $slug = $this->urldecode( $_GET['plugin'] );
			...
			if ( $this->can_plugin_activate( $slug ) ) {
				...
				if ( is_wp_error( $activate ) ) {
					...
					if ( ! $automatic ) {
						...
						if ( isset( $_POST['action'] ) && wp_verify_nonce( sanitize_key( $_POST['action'] ), 'tgmpa-nonce' ) ) {
		...
		protected function is_core_update_page() {
			...
			if ( 'update-core' === $screen->base ) {
				// Core update screen.
				return true;
			} elseif ( 'plugins' === $screen->base && ! empty( $_POST['action'] ) && ! wp_verify_nonce( sanitize_key( $_POST['action'] ), 'tgmpa-nonce' ) ) { // phpcs:ignore Standard.Category.SniffName.ErrorCode
				// Plugins bulk update screen.
				return true;
			} elseif ( 'update' === $screen->base && ! empty( $_POST['action'] ) && ! wp_verify_nonce( sanitize_key( $_POST['action'] ), 'tgmpa-nonce' ) ) { // phpcs:ignore Standard.Category.SniffName.ErrorCode
		...
		public function process_bulk_actions() {
			...
			if ( 'tgmpa-bulk-install' === $this->current_action() || 'tgmpa-bulk-update' === $this->current_action() ) {
				...
				if ( isset( $_POST['plugin'] ) && is_array( $_POST['plugin'] ) ) {
					$plugins_to_install = array_map( 'sanitize_text_field', wp_unslash( $_POST['plugin'] ) );
				} elseif ( isset( $_POST['plugin'] ) && is_string( $_POST['plugin'] ) ) {
					// Received via Filesystem page - un-flatten array (WP bug #19643).
					$plugins_to_install = array_map( 'sanitize_text_field', explode( ',', sanitize_text_field( wp_unslash( $_POST['plugin'] ) ) ) );
				}
			...
			if ( 'tgmpa-bulk-activate' === $this->current_action() ) {
				...
				if ( isset( $_POST['plugin'] ) ) {
					$plugins = array_map( 'urldecode', array_map( 'sanitize_text_field', (array) wp_unslash( $_POST['plugin'] ) ) );
